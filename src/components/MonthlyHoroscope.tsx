...
export default function MonthlyHoroscope({ isOpen, onClose }: { isOpen: boolean; onClose: () => void; }) {
  ...
  const getHoroscopeData = (_signName: string) => {
    // Sample monthly horoscope data
    return {
      love: "This month emphasizes emotional connections and romantic growth. Singles may find meaningful relationships, while couples deepen their bonds. Focus on open communication and mutual understanding.",
      personal: "Personal development is highlighted this month. Consider learning new skills, pursuing hobbies, or working on self-improvement goals. This is an excellent time for personal transformation.",
      career: "Career advancement opportunities abound. Your hard work and dedication will be recognized. Consider networking, taking on leadership roles, or exploring new professional avenues.",
      health: "Holistic health takes priority this month. Balance physical exercise with mental wellness practices. Pay attention to diet, sleep patterns, and stress management for optimal well-being.",
      emotions: "Emotional intelligence and intuition are your greatest assets. Trust your inner voice in decision-making. Practice mindfulness and emotional self-care throughout the month.",
      lucky: "Lucky numbers: 2, 6, 9, 11. Lucky colors: Purple, Gold. Lucky gemstone: Amethyst. Lucky days: Tuesdays and Fridays. Best time for important decisions: Between 10 AM - 12 PM.",
      travel: "Travel opportunities bring growth and new experiences. Long-distance journeys may be particularly beneficial. Consider cultural or spiritual travel destinations.",
      remedies: "Perform daily meditation focusing on your goals. Wear gold jewelry on special occasions. Plant basil or money plant at home for positive energy. Chant 'Om Shreem Maha Lakshmiyei Namaha' for prosperity."
    };
  };
    ...
    <div className="fixed inset-0 z-[64] overflow-y-auto bg-[var(--primary-bg)]">
      ...
      <div className="relative h-[32vh] md:h-[40vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-[var(--text-primary)] mb-3 tracking-tight drop-shadow-xl animate-slideInUp">{lang === 'hi' ? 'मासिक राशिफल' : 'Monthly Horoscope'}</h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
              {zodiacSigns.map((sign, index) => (
                <button
                  ...
                  className="glass rounded-2xl border border-[var(--border-color)] hover:border-[var(--accent-color)]/50 p-4 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-[var(--accent-color)]/20 group"
                >
                  <div className="text-center">
                    <div className="text-3xl mb-2 group-hover:scale-110 transition-transform duration-300">
                      {sign.symbol}
                    </div>
                    <h3 className="text-[var(--text-primary)] font-bold text-sm mb-1 group-hover:text-[var(--accent-color)] transition-colors">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-[var(--text-secondary)]">
                    {lang === 'hi' ? 'मासिक राशिफल' : 'Monthly Horoscope'} - {new Date().toLocaleString('default', { month: 'long', year: 'numeric' })}
                  </p>
                </div>
              </div>
              <button
