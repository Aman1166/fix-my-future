...
export default function CartDrawer() {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-gray-800/60 backdrop-blur-sm z-[55]" onClick={() => { setIsCartOpen(false); setStep('cart'); }}></div>
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-gray-800 z-[56] shadow-2xl border-l border-white/10 flex flex-col animate-slideInRight">
        {/* Header */}
        <div className="bg-gradient-to-r from-gray-800 to-gray-900 p-5 flex items-center justify-between border-b border-white/10">
          <div className="flex items-center space-x-3">
            {step === 'checkout' && (
              <button onClick={() => setStep('cart')} className="text-gray-400 hover:text-white mr-2">
          ...
          <button onClick={() => { setIsCartOpen(false); setStep('cart'); }} className="text-gray-400 hover:text-white text-xl font-bold">✕</button>
        </div>
        ...
        {step === 'success' && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <div className="bg-gray-800 rounded-xl p-4 border border-white/5 w-full max-w-xs">
              <p className="text-xs text-gray-400 font-bold">Order ID: <span className="text-amber-500">#FMF{Math.floor(Math.random() * 90000 + 10000)}</span></p>
              <p className="text-xs text-gray-400 font-bold mt-1">Estimated Delivery: <span className="text-green-400">3-5 Business Days</span></p>
            </div>
          </div>
        )}
          ...
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.id} className="bg-gray-800/80 rounded-xl p-3 flex items-center space-x-3 border border-white/5 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 bg-gray-700 border border-white/10 rounded-lg flex items-center justify-center text-gray-300 hover:bg-slate-700 text-sm font-bold">−</button>
            ...
            {/* Cart Footer */}
            {cart.length > 0 && (
              <div className="border-t border-white/10 bg-gray-800/80 p-4 space-y-3">
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between text-gray-400"><span>Subtotal</span><span>₹{totalPrice}</span></div>
                  ...
                  <div className="flex justify-between text-gray-400"><span>GST (18%)</span><span>₹{gst}</span></div>
                  <div className="flex justify-between text-white font-black text-base pt-2 border-t border-white/5"><span>Total</span><span className="text-amber-500">₹{grandTotal}</span></div>
                </div>
        ...
        {step === 'checkout' && (
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="bg-gray-800/80 rounded-xl p-4 border border-white/5 space-y-3">
              ...
              {[
                ...
                <button
                  ...
                  className={`w-full flex items-center space-x-3 p-3 rounded-lg border transition-all text-left ${paymentMethod === method.id ? 'border-amber-500 bg-amber-700/10' : 'border-white/10 hover:border-white/20'}`}
                >
                  <span className="text-xl">{method.icon}</span>
                  <span className={`text-sm font-bold ${paymentMethod === method.id ? 'text-amber-500' : 'text-gray-300'}`}>{method.label}</span>
            ...
            <div className="bg-gray-800/80 rounded-xl p-4 border border-white/5 space-y-2">
              ...
              {cart.map(item => (
                <div key={item.id} className="flex justify-between text-xs text-gray-400">
                  ...
                  <span className="font-bold text-white flex-shrink-0">₹{item.priceNum * item.quantity}</span>
                </div>
              ))}
              <div className="border-t border-white/5 pt-2 mt-2 space-y-1 text-xs">
                <div className="flex justify-between text-gray-400"><span>Subtotal</span><span>₹{totalPrice}</span></div>
                <div className="flex justify-between text-gray-400"><span>Shipping</span><span className={shipping === 0 ? 'text-green-400' : ''}>{shipping === 0 ? 'FREE' : `₹${shipping}`}</span></div>
                <div className="flex justify-between text-gray-400"><span>GST</span><span>₹{gst}</span></div>
                <div className="flex justify-between text-white font-black text-base pt-1 border-t border-white/5"><span>Grand Total</span><span className="text-amber-500">₹{grandTotal}</span></div>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full font-black py-4 rounded-xl shadow-xl text-sm tracking-wider uppercase active:scale-[0.98] transition-all ${paymentMethod ? 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white shadow-green-500/10' : 'bg-gray-700 text-stone-500 cursor-not-allowed'}`}
            >
