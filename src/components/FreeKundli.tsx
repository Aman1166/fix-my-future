    ...
    <div className="fixed inset-0 z-[64] overflow-y-auto bg-[var(--primary-bg)]">
      ...
      <div className="relative h-[32vh] md:h-[40vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          <span className="text-5xl md:text-7xl mb-4 animate-float"><i className="fas fa-chart-bar text-[var(--accent-color)]"></i></span>
        ...
        {!showResult ? (
          ...
          <div className="glass-heavy rounded-2xl border border-[var(--border-color)] p-8">
            ...
            <div className="glass rounded-xl p-6 mb-6">
              <h4 className="text-[var(--accent-color)] font-bold text-lg mb-4">
                {lang === 'hi' ? 'बुनियादी विश्लेषण' : 'Basic Analysis'}
              </h4>
              <div className="grid md:grid-cols-2 gap-4 text-gray-300">
                <div>
                  <span className="font-bold text-[var(--text-primary)]">{lang === 'hi' ? 'नाम:' : 'Name:'}</span> {formData.name}
                </div>
                <div>
                  <span className="font-bold text-[var(--text-primary)]">{lang === 'hi' ? 'लिंग:' : 'Gender:'}</span> {formData.gender}
                </div>
                <div>
                  <span className="font-bold text-[var(--text-primary)]">{lang === 'hi' ? 'जन्म तिथि:' : 'Birth Date:'}</span> {formData.birthDate}
                </div>
                <div>
                  <span className="font-bold text-[var(--text-primary)]">{lang === 'hi' ? 'जन्म समय:' : 'Birth Time:'}</span> {formData.birthTime}
                </div>
                <div className="md:col-span-2">
                  <span className="font-bold text-[var(--text-primary)]">{lang === 'hi' ? 'जन्म स्थान:' : 'Birth Place:'}</span> {formData.birthPlace}
                </div>
              </div>
            </div>
