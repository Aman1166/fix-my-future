        ...
        <div className="glass-heavy rounded-2xl border border-[var(--border-color)] p-8">
          {!result ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="text-center"><button type="submit" className="bg-gradient-to-r from-amber-500 to-amber-600-500 hover:from-amber-600 hover:to-amber-600-600 text-gray-800 font-black py-4 px-8 rounded-xl transition-all shadow-xl hover:shadow-[var(--accent-color)]/30 text-lg">{lang === 'hi' ? 'इष्ट देवता खोजें' : 'Find Ishta Devata'}</button></div>
            </form>
          ) : (
            <div className="text-center">
              ...
              <button onClick={() => setResult(null)} className="btn-accent text-gray-800 font-bold text-[var(--text-primary)] font-bold py-2 px-6 rounded-xl transition-all">{lang === 'hi' ? 'दोबारा खोजें' : 'Find Again'}</button>
            </div>
          )}
        </div>
      </div>
